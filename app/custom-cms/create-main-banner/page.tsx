...
function BannerContent({
  ...
  // Sizes for mobile scaling - don't scale if already in a device frame
  const mobileScale = devicePreview === 'mobile' && !isInDeviceFrame ? 0.6 : 1
  ...
  React.useEffect(() => {
    ...
    } else if (bannerData.bannerImageUrl || bannerData.image_url || bannerData.imageUrl) {
      // If using a remote image URL
      if (blobUrlRef.current) {
        URL.revokeObjectURL(blobUrlRef.current)
        blobUrlRef.current = null
      }
      setImageSrc(bannerData.bannerImageUrl || bannerData.image_url || bannerData.imageUrl)
    } else {
      // No image
      if (blobUrlRef.current) {
        URL.revokeObjectURL(blobUrlRef.current)
  ...
  }, [bannerData.bannerImage, bannerData.bannerImageUrl, bannerData.image_url, bannerData.imageUrl])
...
function MainBannerCanvas({
  ...
  const mobileWidth = isFullScreen ? 500 : 375
  const mobileHeight = isFullScreen ? 900 : 320
  const desktopWidth = isFullScreen ? 900 : '100%'
  const desktopHeight = isFullScreen ? 500 : 320
          ...
          >
            ...
            >
              <button
                ...
                } rounded-lg transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105 active:scale-95 px-3 py-1.5 sm:px-4 sm:py-2 md:px-5 md:py-2.5 lg:px-6 lg:py-3 xl:px-8 xl:py-3.5 text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl leading-none sm:leading-tight md:leading-snug lg:leading-normal`}
                style={{
...
export default function CreateBannerPage() {
  ...
  const [bannerData, setBannerData] = useState({
    ...
    ctaPosition: { x: 50, y: 65 },
    titleMarginBottom: 12,
    subtitleMarginBottom: 20,
    // Custom font size properties - Device Specific
    titleCustomFontSize: '48',
    subtitleCustomFontSize: '24',
    ctaCustomFontSize: '18',
    ...
    setBannerData((prev) => {
      ...
      if (prev.showSubtitle && !next.showSubtitle) {
        ...
        if (!next.subtitlePosition) {
          next.subtitlePosition = { x: 50, y: 40 }
          console.log(
  ...
  const handleImageUpload = (file: File) => {
    ...
    // If there was a previous image URL, store it for cleanup
  ...
  // Image upload is now handled during publish - no automatic upload
  ...
  const generateBannerHtml = (
    ...
    // Smart font size handling: use same properties as React component
    const titleFontSize = bannerData.showTitle
      ? bannerData.titleCustomFontSize || 72
      : 72
    const subtitleFontSize = bannerData.showSubtitle
      ? bannerData.subtitleCustomFontSize || 52
      : 52
    ...
    const buttonStyles = `
      ...
      padding: ${bannerData.buttonPaddingTop || 0}px ${
      bannerData.buttonPaddingRight || 0
    }px ${bannerData.buttonPaddingBottom || 0}px ${
      bannerData.buttonPaddingLeft || 0
    }px;
...
@media (max-width: 768px) {
  .banner-title {
    font-size: ${Math.max(
      parseInt(bannerData.titleCustomFontSize) * 0.7,
      16
    )}px !important;
  }
  .banner-subtitle {
    font-size: ${Math.max(
      parseInt(bannerData.subtitleCustomFontSize) * 0.7,
      12
    )}px !important;
  }
  .banner-cta {
    font-size: ${Math.max(
      parseInt(bannerData.ctaCustomFontSize) * 0.7,
      10
    )}px !important;
  }
}
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      if (isEditMode && editId) {
        response = await fetch(`/api/banners/${editId}`, {
          method: 'PUT',
      ...
      if (result.success) {
        toast({
          title: 'Draft Saved',
          description: 'Banner saved as draft successfully!',
        })
        router.push('/custom-cms/main-banners')
      } else {
        toast({
          title: 'Error',
  ...
  const handlePublishBanner = async () => {
    ...
    try {
      ...
      if (selectedBannerImage) {
        try {
          ...
          if (existingBannerImageUrl) {
            try {
              await fetch(`/api/upload/main-banner?url=${encodeURIComponent(existingBannerImageUrl)}`, {
                method: 'DELETE',
              })
              console.log('Deleted existing banner image:', existingBannerImageUrl)
            } catch (deleteError) {
              console.error('Error deleting existing banner image:', deleteError)
            }
          }
          ...
          const uploadResponse = await fetch('/api/upload/main-banner', {
            method: 'POST',
            body: uploadFormData,
          })
          ...
          if (!uploadResponse.ok) {
            ...
            toast({
              title: 'Warning',
              description: `Banner published but image upload failed: ${uploadError.error || uploadError.details || 'Unknown error'}. You can update the image later.`,
              variant: 'default',
            })
          } else {
            const uploadData = await uploadResponse.json()
            console.log('Banner image uploaded successfully:', uploadData.url)
            finalImageUrl = uploadData.url
          }
        } catch (uploadError) {
          console.error('Error during banner image upload:', uploadError)
          const errorMessage = uploadError instanceof Error ? uploadError.message : 'Unknown error'
          toast({
            title: 'Warning',
            description: `Banner published but image upload failed: ${errorMessage}. You can update the image later.`,
            variant: 'default',
          })
        }
      }
      ...
      if (result.success) {
        toast({
          title: isEditMode ? 'Banner Updated' : 'Banner Published',
      ...
      console.error(
        isEditMode ? 'Error updating banner:' : 'Error publishing banner:',
        error
      )
      toast({
        title: 'Error',
      ...
      .then((result) => {
        ...
        setBannerData((prev) => ({
          ...
          subtitlePosition: banner.subtitlePosition ||
            subtitlePosition || { x: 50, y: 40 },
          ctaPosition: banner.ctaPosition || ctaPosition || { x: 50, y: 65 },
                ...
                <div className='relative'>
                  <Button
                    ...
                    disabled={
                      isPublishing ||
                      !bannerData.name.trim() ||
                      (!selectedBannerImage && !bannerData.bannerImageUrl && !bannerData.image_url && !bannerData.imageUrl)
                    }
