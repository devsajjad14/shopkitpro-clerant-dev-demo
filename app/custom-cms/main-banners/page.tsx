  ...
  const handlePublishBanner = async (id: number) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({ title: 'Success', description: 'Banner published successfully!' })
      } else {
        toast({ title: 'Error', description: result.error || 'Failed to publish banner', variant: 'destructive' })
      }
    } catch (error) {
      console.error('Error publishing banner:', error)
      toast({ title: 'Error', description: 'Failed to publish banner', variant: 'destructive' })
    } finally {
      ...
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='text-gray-600 dark:text-gray-400'>
            Manage your main banner content
          </p>
        </div>
        <button
        ...
        {currentBanners.length === 0 ? (
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6'>
            {currentBanners.map((banner) => {
              const imageUrl = banner.image_url || banner.imageUrl || '/placeholder-banner.jpg';
              return (
                <motion.div
                  key={banner.id}
                  initial={{ opacity: 0, y: 24 }}
                  animate={{ opacity: 1, y: 0 }}
                  whileHover={{ scale: 1.02, boxShadow: '0 20px 40px 0 rgba(31,41,55,0.15)' }}
                  className='bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-lg hover:shadow-2xl transition-all duration-300 group flex flex-col overflow-hidden min-h-[280px] relative'
                  style={{ minHeight: 280 }}
                >
                  <img
                    src={imageUrl}
                    alt={banner.name}
                    className='mx-auto w-full aspect-[16/9] object-cover rounded-t-2xl shadow-md'
                  />
                  <div className='absolute top-4 right-4 flex items-center gap-2 z-10'>
                    <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-semibold shadow-md ${getStatusColor(banner.status)}`}>{banner.status}</span>
                  </div>
                  <div className='flex-1 flex flex-col justify-between p-4 gap-3'>
                    <h3 className='text-lg font-bold text-gray-900 dark:text-white text-center tracking-tight leading-tight truncate mb-2'>{banner.name}</h3>
                    <div className='flex items-center justify-between gap-3 mt-2'>
                      <span className='inline-flex items-center gap-1 px-3 py-1 rounded-full bg-gray-50 dark:bg-gray-800 text-sm font-medium text-gray-700 dark:text-gray-200 shadow-sm'>
                        <FiClock className='w-4 h-4 opacity-70' />{new Date(banner.updatedAt).toLocaleDateString()}
                      </span>
                      <div className='flex items-center gap-1'>
                        <button
                          title='View'
                          onClick={() => setViewBanner(banner)}
                          className='w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-gray-200 text-gray-500 shadow-md hover:bg-gray-50 hover:shadow-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-gray-200/60'
                        ...
                        <button
                          title='Edit'
                          onClick={() => router.push(`/custom-cms/create-main-banner?id=${banner.id}`)}
                          className='w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-gray-200 text-gray-500 shadow-md hover:bg-gray-50 hover:shadow-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-gray-200/60'
                        >
                        ...
                        {banner.status === 'draft' && (
                          <button
                            ...
                            className='w-10 h-10 flex items-center justify-center rounded-xl bg-green-50 border border-green-200 text-green-600 shadow-md hover:bg-green-100 hover:shadow-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-green-200/60 disabled:opacity-50'
                          >
                        ...
                        <button
                          ...
                          className='w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-gray-200 text-gray-500 shadow-md hover:bg-gray-50 hover:shadow-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-gray-200/60 disabled:opacity-50'
                        >
                  ...
                  >
                    <td className='px-6 py-4'>
                      <div className='flex items-center gap-4'>
                        <img
                          src={banner.image_url || banner.imageUrl || '/placeholder-banner.jpg'}
                          alt={banner.name}
                    ...
                    <td className='px-6 py-4 text-right'>
                      <div className='flex items-center justify-end gap-2'>
                        ...
                        <button
                          onClick={() =>
                            router.push(
                              `/custom-cms/create-main-banner?id=${banner.id}`
                            )
                          }
