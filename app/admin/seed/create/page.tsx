...
export default function CreateSeedData() {
  ...
  const handleCreate = async () => {
    if (!confirm('Are you sure you want to create seed data? This will add new records to the database.')) {
      return;
    }
  ...
  const getStatusIcon = (status: TableStatus) => {
    if (!isLoading && status === 'pending') {
      return null; // Don't show any icon when not loading and status is pending
    }
  ...
  return (
    <div className="container mx-auto py-8">
      <Card className="p-6 bg-gradient-to-br from-white to-gray-50 shadow-lg">
        <h1 className="text-3xl font-bold mb-6 bg-gradient-to-r from-green-600 to-green-800 bg-clip-text text-transparent">
          Create Seed Data
        </h1>
        <div className="space-y-6">
          <div className="space-y-4">
            <Label className="text-lg font-semibold">Information</Label>
            <p className="text-sm text-gray-600">
              This action will create seed data in the following tables in order:
            </p>
            ...
            <p className="text-sm text-gray-600 mt-2">
              Note: Tables are created in this specific order to maintain database integrity and respect foreign key relationships.
            </p>
          </div>
          <Button 
