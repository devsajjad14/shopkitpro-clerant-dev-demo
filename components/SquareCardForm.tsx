...
export interface SquareCardFormRef {
  getCardDetails: () => {
    cardNumber: string
    expiryMonth: string
    expiryYear: string
...
const SquareCardForm = forwardRef<SquareCardFormRef, SquareCardFormProps>(({ 
  onCardDetails, 
  onError,
  disabled = false 
}, ref) => {
  ...
  const validateAndSendCardDetails = useCallback(() => {
    ...
    timeoutRef.current = setTimeout(() => {
      ...
      if (cardNumber && expiryMonth && expiryYear && cvc && nameOnCard) {
        // More flexible card number validation for Square sandbox
        const cleanCardNumber = cardNumber.replace(/\s/g, '')
        if (!/^\d{13,19}$/.test(cleanCardNumber)) {
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div 
        className="relative h-48 bg-gradient-to-br from-slate-800 via-slate-700 to-slate-900 rounded-2xl p-6 text-white overflow-hidden shadow-2xl"
            ...
            <input
              ...
              className={`block w-full rounded-xl border-2 shadow-sm text-lg px-4 py-4 transition-all duration-300 font-mono ${
                focusedField === 'cardNumber' 
                  ...
                  : 'border-gray-200 focus:border-blue-500 focus:shadow-blue-100 focus:bg-blue-50'
